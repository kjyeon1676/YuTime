...
socket.on('result_search_item',function(item){
    ...
    var $CourseNumber=$('<td style="text-align:center;">'+item.CourseNumber+'</td>').appendTo($item);
    var $CourseName=$('<td>'+item.CourseName+'</td>').appendTo($item);
    var $Professor=$('<td style="text-align:center;">'+item.Professor+'</td>').appendTo($item);
    ...
    var $Time=$('<td>'+item.Time+'</td>').appendTo($item);
    var $Classroom=$('<td>'+item.Classroom+'</td>').appendTo($item);
...
function test(event,data){
    if(timetable_mydata_list.data==undefined){
    timetable_mydata_list[data]=timetable_data_list[data];
    
    move_list_to_table(event,data);
...
function move_list_to_table(mouseevent,data){
    ...
    for(var j=0;j<halfcnt;++j){
        ...
        if($(tg).hasClass(data.CourseNumber+data.Department)){
            ...
            break;
                ...
                alert('시간 중복입니다.');}
                if(mouseevent=='autoconfig'){console.log('중복') delete (timetable_mydata_list[data.CourseNumber+data.Department])}
                return;
        }
        tempmin+=30;
    }
    ...
    halfcnt/=30;
        ...
        if(mouseevent=='onclick'||mouseevent=='initialize'||mouseevent=='remove'){
        ...
        if(mouseevent=='initialize'||(mouseevent=='mouseover'&&!$(target).hasClass('choose'))){
